...
class ScanController:
    ...
    def _scan_target(self, url: str, headers: Dict[str, str], timeout: int = 10, 
                    delay: float = 0, retries: int = 3, payloads: Optional[List[str]] = None,
                    result_list: Optional[List[ScanResult]] = None) -> None:
        # Use provided payloads or default ones
        test_origins = payloads if payloads else [
            "https://example.com",
            "http://malicious.com",
            "null",
            "http://localhost",
        ...
        self.logger.info(f"Starting scan for {url} with {retries} retries and {timeout}s timeout")
        ...
        for origin in test_origins:
            ...
            while attempt <= retries:
                try:
                    ...
                    self.logger.debug(f"Testing origin: {origin} (attempt {attempt + 1}/{retries + 1})")
                    response = requests.get(url, headers=request_headers, timeout=timeout)
                    ...
                    attempt += 1
                    if attempt > retries:
                        self.logger.error(f"Request failed for origin {origin} after {retries} attempts: {str(e)}")
                    else:
    ...
    def _worker(self, queue: Queue, headers: Dict[str, str], delay: float, 
               timeout: int, retries: int, payloads: Optional[List[str]], 
               results: List[ScanResult], result_lock: threading.Lock) -> None:
        while not queue.empty():
            try:
                url = queue.get()
                self._scan_target(
    ...
    def scan_single_target(self, domain: str, threads: int = 5, retries: int = 3,
                          delay: float = 0.3, timeout: int = 30, 
                          cookies: Optional[str] = None,
                          payloads: Optional[List[str]] = None) -> ScanResponse:
        self.logger.info(f"Initiating single target scan for {domain}")
        try:
            headers = {}
            if cookies:
    ...
    def scan_batch_targets(self, domains: List[str], threads: int = 5, retries: int = 3,
                          delay: float = 0.3, timeout: int = 30, 
                          cookies: Optional[str] = None,
                          payloads: Optional[List[str]] = None) -> ScanResponse:
        self.logger.info(f"Initiating batch scan for {len(domains)} targets with {threads} threads")
        self.logger.debug(f"Scan parameters - retries: {retries}, delay: {delay}s, timeout: {timeout}s")
        ...
        try:
            ...
            for _ in range(threads):
                t = threading.Thread(
                    target=self._worker,
                    args=(queue, headers, delay, timeout, retries, payloads, results, result_lock)
                )
                t.daemon = True
                worker_threads.append(t)
                t.start()
